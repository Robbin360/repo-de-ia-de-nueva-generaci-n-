...
def main() -> None:
    ...
    manifest = build_quarantined_replay_manifest([first, second], [], rock_hash)
    ...
    fails(lambda: review_event(event("expired-en", "expired-source", 0.5, ttl_observations=0), []), "TTL")
    holdout_event = event("holdout-en", "holdout-source", 0.5)
    fails(lambda: review_event(holdout_event, [holdout_event["source_sha256"]]), "holdout")
    fails(lambda: build_quarantined_replay_manifest([first, dict(first)], [], rock_hash), "duplicado")
