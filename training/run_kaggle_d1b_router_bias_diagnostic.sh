...
# No ejecutar sin una autorización futura, explícita y separada para D1B.
if [[ "${AETHEL_D1B_RUN_AUTHORIZED:-NO}" != "YES" ]]; then
  echo "D1B bloqueado: AETHEL_D1B_RUN_AUTHORIZED debe ser YES." >&2
  exit 2
fi
if [[ "${AETHEL_D1B_GPU_AUTHORIZED:-NO}" != "YES" ]]; then
  echo "D1B bloqueado: AETHEL_D1B_GPU_AUTHORIZED debe ser YES." >&2
  exit 2
fi
if [[ -n "${AETHEL_RESUME_CHECKPOINT:-}" ]]; then
  echo "D1B bloqueado: no acepta AETHEL_RESUME_CHECKPOINT ni reanudación de E0/D1A." >&2
  exit 2
fi
if [[ "${AETHEL_D1B_ALLOW_PYTORCH_FALLBACK:-NO}" != "YES" ]]; then
  echo "D1B bloqueado: el fallback PyTorch experimental requiere autorización separada." >&2
  exit 2
fi

SOURCE_DIR="${AETHEL_SOURCE_DIR:?Falta AETHEL_SOURCE_DIR}"
DATA_DIR="${AETHEL_DATA_DIR:?Falta AETHEL_DATA_DIR}"
OUTPUT_DIR="${AETHEL_D1B_OUTPUT_DIR:?Falta AETHEL_D1B_OUTPUT_DIR}"
...
if [[ ! -f "$RELEASE_FILE" ]] || ! grep -Fq "\"release\": \"$EXPECTED_RELEASE\"" "$RELEASE_FILE"; then
  echo "D1B bloqueado: el release fuente exacto $EXPECTED_RELEASE no está montado." >&2
  exit 2
fi
if [[ -e "$OUTPUT_DIR" ]]; then
  echo "D1B bloqueado: AETHEL_D1B_OUTPUT_DIR ya existe; se requiere una salida nueva y vacía." >&2
  exit 2
fi
for required in \
  ...
  [[ -f "$required" ]] || { echo "D1B bloqueado: falta $required." >&2; exit 2; }
done

mkdir -p "$OUTPUT_DIR"
python3 "$SOURCE_DIR/training/validate_aethel_train_only_mount.py" \
  --package-dir "$DATA_DIR" \
  --report "$OUTPUT_DIR/d1b_train_only_preflight.json"
...
python3 "$SOURCE_DIR/engine/train_aethel_gpu.py" \
  ...
  --memory-slots 512 --replay-capacity 8192 --router-bias-step 0.01 --router-bias-limit 0.5 \
