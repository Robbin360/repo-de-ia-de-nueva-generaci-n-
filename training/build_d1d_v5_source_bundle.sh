...
# Construye únicamente un bundle local de código D1D V5. No lee Dataset,
# no descarga/publica recursos y rechaza datos, pesos, métricas y bytecode.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OUTPUT_DIR="${1:-$ROOT/../aethel-private-transfer}"
PACKAGE_NAME="aethel-nextgen-source-d1d-v1-router-entropy-train-only"
RELEASE_FILE="$ROOT/training/aethel_d1d_v5_source_release.json"
...
if [[ "$SOURCE_RELEASE" != "$EXPECTED_RELEASE" ]]; then
  echo "El marcador local no corresponde al release D1D V5 esperado." >&2
  exit 2
fi
...
if find "$SOURCE_ROOT" -type f \( -name '*.pt' -o -name '*.pth' -o -name '*.ckpt' -o -name '*.safetensors' -o -name '*.jsonl' -o -name '*.jsonl.gz' -o -name '*.pyc' \) -print -quit | grep -q .; then
  echo "El bundle V5 contiene un dato o artefacto prohibido." >&2
  exit 2
fi

if ! grep -Fq "\"release\": \"$EXPECTED_RELEASE\"" "$SOURCE_ROOT/training/aethel_d1d_v5_source_release.json"; then
  echo "El bundle V5 no preservó el marcador de release exacto." >&2
  exit 2
fi
...
cat > "$MANIFEST" <<EOF
{
  "schema": "aethel-d1d-v5-source-bundle/v1",
...
printf 'D1D_V5_BUNDLE_LOCAL_READY\nTAR=%s\nZIP=%s\nMANIFEST=%s\n' "$TAR_ARCHIVE" "$ZIP_ARCHIVE" "$MANIFEST"
