...
for path in files:
    try:
        ...
        required = {
            ...
            "wikimedia_fallback": b"wikimedia-enwiki-multistream-20260805-p1p41242" in manifest_bytes,
...
# Si Kaggle montó copias duplicadas, sólo permitimos la que incorpora el
# respaldo oficial de Wikimedia. Después priorizamos Project Gutenberg, OLC,
# completitud, tamaño y hash; así ninguna copia anterior puede ganar.
candidates.sort(
    key=lambda item: (
        item[1]["wikimedia_fallback"],
...
if not details["gutenberg_source"] or not details["wikimedia_fallback"]:
    raise RuntimeError(
        "El selector no encontró un bundle con project-gutenberg-en y el "
        "respaldo wikimedia-enwiki-multistream-20260805-p1p41242. "
        "No se permite continuar con una copia antigua."
    )
print(
    "Bundles válidos detectados: "
    f"{len(candidates)}; seleccionado: {bundle} "
    f"(wikimedia_fallback={details['wikimedia_fallback']}, "
...
if len(candidates) > 1:
    ...
    for discarded, discarded_details, discarded_digest in candidates[1:]:
        print(
            f"- {discarded} (wikimedia_fallback={discarded_details['wikimedia_fallback']}, "
            f"project_gutenberg={discarded_details['gutenberg_source']}, "
